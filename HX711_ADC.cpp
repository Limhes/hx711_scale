...
HX711_ADC::HX711_ADC() //constructor
{
	// SCK = PB1 = output:
	DDRB |= (1 << DDB1);
	// DOUT = PB0 = input:
...
{
	char dout = (PINB & (1 << PINB0)) >> PINB0; //check if conversion is ready
	if (!dout) 
	{
		conversion24bit();
		lastDoutLowTime = timing::millis();
...
bool HX711_ADC::refreshDataSet()
{
	...
	while ( s > 0 ) {
		...
		if ((PINB & (1 << PINB0)) >> PINB0 == 0) { // HX711 dout pin is pulled low when a new conversion is ready
